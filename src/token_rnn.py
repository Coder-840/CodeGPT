# File: src/token_rnn.py
import numpy as np
import re

class TokenRNN:
    """
    Token-level RNN for self-contained Python code generation.
    """
    def __init__(self, vocab=None, hidden_size=128, learning_rate=0.1):
        self.hidden_size = hidden_size
        self.learning_rate = learning_rate
        self.token_pattern = re.compile(r'\w+|[^\s\w]')
        
        if vocab is None:
            self.vocab = []
        else:
            self.vocab = list(vocab)
        self.token_to_ix = {t:i for i,t in enumerate(self.vocab)}
        self.ix_to_token = {i:t for i,t in enumerate(self.vocab)}
    ...
    def _update_vocab(self, text):
        ...
        for t in tokens:
            if t not in self.token_to_ix:
                idx = len(self.vocab)
                self.vocab.append(t)
                self.token_to_ix[t] = idx
                self.ix_to_token[idx] = t
                self.vocab_size = len(self.vocab)
        if self.Wxh is None:
            self._init_weights()
    ...
    def online_train(self, text, hprev=None):
        ...
        tokens = self.tokenize(text)
        if len(tokens)<2: return hprev,0
        inputs = [self.token_to_ix[t] for t in tokens[:-1]]
        targets = [self.token_to_ix[t] for t in tokens[1:]]
        if hprev is None:
